...
Panel {
  ...
  // Absolute path to this plugin's directory, so bin/battery-details can be
  // launched without depending on it being on PATH.
  ...
  Process {
    id: detailsProc
    command: [root.pluginDir + "bin/battery-details"]
    ...
    PanelKeyCatcher {
      ...
      Column {
        ...
        Row {
          ...
          Column {
            // Hidden on multi-battery machines: omarchy-battery-status reports
            // the first cell only, so these would be BAT0's numbers wearing a
            // machine-wide label. The per-battery cards below carry them.
  ...
  component BatteryCard: Column {
    ...
    Item {
      ...
      // Named by bin/battery-details: the T480 is a Power Bridge machine, so
